function [W V] = BAMRlLearning8fromfile()
...
loadfile = input('Please input name of the file to load, Master.', 's')
loadediter = input('How many iterations were in that saved file, Master?', 's')
load(loadfile)
OuvrirNXT
nbIter = input('How many iterations shall I perform, Master?')
global h j 
...
for i=1:nbIter
    ...
    if mod(i,10) == 0
        savefile = ['WV' num2str(i+loadediter) 'iter.mat'];
        save(savefile)
    end
...
%Pourquoi on ferme les senseurs?
%% Éteindre les sensors à la toute fin
%CloseSensor(SENSOR_1, h);
%CloseSensor(SENSOR_4, j);
